...
public class GlobalExceptionHandler extends ResponseEntityExceptionHandler {
     ...
     * @param status The HttpStatusCode for the response.
     * @param request The WebRequest object for the response.
     * @return A ResponseEntity containing an ErrorResponse object with the
     * error code and localized message, and the HTTP status code.
     */
    @Override
    protected ResponseEntity<Object> handleMethodArgumentNotValid(MethodArgumentNotValidException ex, HttpHeaders headers, HttpStatusCode status, WebRequest request) {

        return new ResponseEntity<>(new ErrorResponse(errorCodeBadRequest, ex.getLocalizedMessage()), HttpStatusCode.valueOf(400));
    }
